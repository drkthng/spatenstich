// Capture Step 2 — North-side photo (2/3).
...
export default function StepNorthScreen(): React.JSX.Element {
  ...
  const handlePhoto = React.useCallback(
    async (uri: string) => {
      ...
      addPhoto(resized);
      router.push('/(app)/capture/step-south');
    },
    [addPhoto, router],
  );
    ...
    <View className="flex-1 bg-[#F9F7F4] dark:bg-[#1C1917]">
      ...
      <View className="flex-1 items-center justify-center">
        ...
        <Text className="text-sm text-stone-500 mt-2">
          {t('capture.step.indicator').replace('{current}', '2').replace('{total}', '3')}
        </Text>
      ...
      <CaptureStepCard
        step={2}
        totalSteps={3}
        instruction={t('capture.step.north')}
      />
    </View>
  );
}
