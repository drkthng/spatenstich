// Capture Step 3 — South-side photo (3/3).
// D-01: guided flow with camera + gallery. D-02: instruction + example.
// Flow: capturePhoto/pickFromGallery -> resizeToMaxMp -> captureStore.addPhoto -> review.
...
export default function StepSouthScreen(): React.JSX.Element {
    ...
    <View className="flex-1 bg-[#F9F7F4] dark:bg-[#1C1917]">
      ...
      <View className="flex-1 items-center justify-center">
        ...
        <Text className="text-sm text-stone-500 mt-2">
          {t('capture.step.indicator').replace('{current}', '3').replace('{total}', '3')}
        </Text>
      ...
      <CaptureStepCard
        step={3}
        totalSteps={3}
        instruction={t('capture.step.south')}
      />
    </View>
  );
}
