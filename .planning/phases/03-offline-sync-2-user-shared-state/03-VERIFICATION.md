...
overrides_applied: 0
gaps:
  - truth: "A photo captured offline is stored locally; when the network returns the photo is uploaded and the AI analysis job is enqueued automatically without user action"
    status: failed
    reason: "uploadPending() is fully implemented in PhotoUploader.ts but is NOT called from registerSyncTriggers(). The NetInfo reconnect handler and AppState foreground handler in SyncTriggers.ts only call getSyncWorker().syncAll() — they never import or invoke uploadPending(). Plan 03-05 must_haves.truths[9] explicitly required 'NetInfo-Reconnect triggert PhotoUploader.uploadPending() parallel zum SyncWorker'. Plan 03-05 SUMMARY acknowledges this as a Known Stub deferred to Plan 03-04 — but Plan 03-04 did not add it either."
    artifacts:
      - path: "app/src/lib/sync/SyncTriggers.ts"
        issue: "registerSyncTriggers() does not import or call uploadPending() from PhotoUploader.ts. NetInfo and AppState reconnect handlers only call getSyncWorker().syncAll()."
    missing:
      - "In registerSyncTriggers(), after getSyncWorker().syncAll(), add: import { uploadPending } from '../photos/PhotoUploader'; then in both the NetInfo online handler and AppState active handler call uploadPending().catch(...) in parallel with syncAll()"
human_verification:
...
**Phase Goal:** Dirk und seine Frau können die App ohne Internet öffnen, den gemeinsamen Plan sehen und bearbeiten, und alle Änderungen (inkl. Foto-Queue) werden bei Reconnect automatisch synchronisiert — LWW bei gleichzeitigen Edits, Sync-Status sichtbar.
**Verified:** 2026-04-25T18:30:00Z
**Status:** gaps_found
...
| SC-2 | A photo captured offline is stored locally; when the network returns the photo is uploaded and the AI analysis job is enqueued automatically without user action | ✗ FAILED | `uploadPending()` in `PhotoUploader.ts` is fully implemented (ArrayBuffer pattern, RPC call) but is **not wired** into `registerSyncTriggers()`. NetInfo and AppState reconnect handlers only call `getSyncWorker().syncAll()`, which handles the garden/profile/vereinsregeln outbox — not photo uploads. |
...
| `app/src/lib/sync/SyncTriggers.ts` | NetInfo + AppState + 500ms debounce triggers | ✓ VERIFIED | registerSyncTriggers() with wasOffline flag; scheduleWriteDebounced with clearTimeout; WRITE_DEBOUNCE_MS = 500 exported. **MISSING: uploadPending() not wired** |
...
| `app/src/lib/photos/PhotoUploader.ts` | uploadPending with ArrayBuffer pattern + RPC | ✓ VERIFIED (artifact) | Function fully implemented with serialisation lock, ArrayBuffer fetch, Storage upload, enqueue_photo_analysis RPC. **ORPHANED — not called from SyncTriggers** |
...
| `uploadPending` | `registerSyncTriggers()` reconnect handler | **MISSING — not imported or called** | ✗ NOT_WIRED | SyncTriggers.ts has no import for PhotoUploader; NetInfo and AppState handlers do not call uploadPending() |
...
| scheduleWriteDebounced uses 500ms constant | `grep -n "WRITE_DEBOUNCE_MS = 500" SyncTriggers.ts` | Line 13 found | ✓ PASS |
...
| uploadPending called on reconnect | `grep -rn "uploadPending" app/src/ (excl tests)` | Only defined in PhotoUploader.ts — **not imported in SyncTriggers.ts** | ✗ FAIL |
...
| SYNC-02 | 03-05 | Foto-Queue works offline; photos stored locally; AI analysis queued on reconnect | ✗ BLOCKED | photoQueueRepo.enqueuePhoto works offline; PhotoUploader.uploadPending fully implemented; **trigger wiring missing** — uploadPending never called on reconnect |
...
|------|---------|----------|--------|
| `app/src/lib/photos/PhotoUploader.ts` | `(supabase as any).rpc(...)` cast for enqueue_photo_analysis | ℹ Info | Type-safety gap; SUMMARY notes to fix after `pnpm gen:types`. Not a runtime blocker. |
| `app/src/lib/migrateLocalToAccount.ts` | `(supabase.rpc as any)('server_now')` cast | ℹ Info | Same root cause — database.ts RPC union missing server_now. Not a runtime blocker. |
| `app/src/lib/sync/SyncTriggers.ts` | `uploadPending` not wired in reconnect handlers | 🛑 Blocker | SYNC-02 / SC-2: photo auto-upload on reconnect does not trigger. Photos queue locally but never upload without user action. |
| `deferred-items.md` | PhotoUploader.test.ts + photoQueueRepo.test.ts pre-existing failures | ⚠ Warning | Test suite reliability impacted; root cause suspected to be mock state pollution. Not blocking SC-2 fix but should be addressed. |
...
**Gap 1 — Photo auto-upload on reconnect not triggered (SC-2 / SYNC-02)**

Root cause: `uploadPending()` was intentionally created as a stub in Plan 03-05 and was supposed to be wired by Plan 03-04. Neither plan added the wiring. The final `SyncTriggers.ts` (committed in `82954d5`) contains no import of `PhotoUploader` and the reconnect handlers (`wasOffline` branch in NetInfo handler, AppState `active` handler) only call `getSyncWorker().syncAll()`.

**Fix required:** Add 3 lines to `registerSyncTriggers()` in `app/src/lib/sync/SyncTriggers.ts`:
1. Import `uploadPending` from `'../photos/PhotoUploader'`  
2. In the NetInfo `wasOffline && isConnected` handler: call `uploadPendingPhotos().catch(...)` after `syncAll()`
3. In the AppState `active` handler: call `uploadPendingPhotos().catch(...)` after `syncAll()`

This matches the Plan 03-05 PLAN.md code snippet exactly (lines 787-808 of the plan spec).

**Pre-existing test failures (not blocking but noted):**

`PhotoUploader.test.ts` and `photoQueueRepo.test.ts` are documented as pre-existing failures in `deferred-items.md`. These are suspected mock state pollution issues, not logic errors in the implementation. They should be fixed as a follow-up.
